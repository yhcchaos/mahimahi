...
StochasticSwitchingLink::StochasticSwitchingLink( const double mean_on_time, const double mean_off_time )
    : link_is_on_( false ),
      on_process_( 1.0 / (US_PER_SECOND * mean_off_time) ), //ms->us
      off_process_( 1.0 / (US_PER_SECOND * mean_on_time) ), //ms->us
...
PeriodicSwitchingLink::PeriodicSwitchingLink( const double on_time, const double off_time )
    : link_is_on_( false ),
      on_time_( bound( US_PER_SECOND * on_time ) ), //ms->us
      off_time_( bound( US_PER_SECOND * off_time ) ), //ms->us
      next_switch_time_( timestamp() )
{
